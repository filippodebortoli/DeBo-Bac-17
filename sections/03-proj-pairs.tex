...
\begin{lem}\label{lem:3-1}
  ...
  \begin{enumerate}
    \item \(\lbrace \alpha_n \circ \beta_n^\star \rbrace_{n \in \mathbb{N}}\) is an ascending \(\omega\)--chain in \(\mathcal{C}(E,D)\); %chktex 21
...
Being \((g,g^\star)\) a projection pair, \(g \circ g^\star \sqsubseteq 1_D\) vacuously holds. If the limit--case \(g \circ g^\star = 1_D\) is reached, then \(E\) and \(D\) are isomorphic and \(g^\star = g^{-1}\). %chktex 8
